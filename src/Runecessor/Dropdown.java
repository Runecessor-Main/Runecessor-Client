...
public enum Dropdown {
	DEFAULT() {
		...
		public void selectOption(int option, Client client, RSInterface rsint) {
			client.stream.createFrame(255);
			client.stream.writeDWord(rsint.id);
			client.stream.method431(option);//unsure for conversion, original was writeByte
			Client.sendString(rsint.message, option);
		}
	},
	KEYBINDING() {
		@Override
		public void selectOption(int option, Client client, RSInterface rsint) {
			int key = rsint.id - 39305;
