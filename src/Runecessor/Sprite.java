...
public final class Sprite extends DrawingArea {
		...
		try {
			...
			for (int h = 0; h < myHeight; h++) {
				for (int w = 0; w < myWidth; w++) {
					totalPixels[(h + anInt1443) * maxWidth + (w + anInt1442)] = myPixels[h * myWidth + w];
				}
			}
	...
	public void drawAdvancedSprite(int i, int j, int transparency) {
		int k = transparency;
		i += anInt1442;
		j += anInt1443;
		...
		if (j + k1 > DrawingArea.getBottomY())
			k1 -= (j + k1) - DrawingArea.getBottomY();
		...
		if (i + l1 > DrawingArea.getBottomX()) {
			int i3 = (i + l1) - DrawingArea.getBottomX();
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
	...
	/**
	 * Use for sprites with transparent backgrounds.
	 * @param i
	 * @param j
	 */
	public void drawAdvancedSprite(int i, int j) {
		drawAdvancedSprite(i, j, 256);
	}
	...
	public Sprite(String img) {
		try {
			Image image = Toolkit.getDefaultToolkit().getImage(ClientConstants.getSpritesLocation() + img + ".png");
			ImageIcon sprite = new ImageIcon(image);
			myWidth = sprite.getIconWidth();
			myHeight = sprite.getIconHeight();
			maxWidth = myWidth;
	...
	public Sprite(String img, String nothing) {
		try {
	...
	public void draw24BitSprite(int x, int y) {
		...
		if (y + height > DrawingArea.getBottomY()) {
			height -= (y + height) - DrawingArea.getBottomY();
		}
		...
		if (x + width > DrawingArea.getBottomX()) {
			int trimRight = (x + width) - DrawingArea.getBottomX();
			width -= trimRight;
			srcStep += trimRight;
			destStep += trimRight;
		}
	...
	public void method346(int i, int j) {
		...
		if (j + j1 > DrawingArea.getBottomY())
			j1 -= (j + j1) - DrawingArea.getBottomY();
		if (i < DrawingArea.topX) {
	...
	public void drawTransparentSprite(int x, int y, int opacity) {
		...
		if (y + k1 > DrawingArea.getBottomY())
			k1 -= (y + k1) - DrawingArea.getBottomY();
		if (x < DrawingArea.topX) {
		...
		if (x + l1 > DrawingArea.getBottomX()) {
			int i3 = (x + l1) - DrawingArea.getBottomX();
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
	...
	public void drawSprite(int x, int y) {
		...
		if (y + j1 > DrawingArea.getBottomY()) {
			j1 -= (y + j1) - DrawingArea.getBottomY();
		}
		if (x < DrawingArea.topX) {
		...
		if (x + k1 > DrawingArea.getBottomX()) {
			int l2 = (x + k1) - DrawingArea.getBottomX();
			k1 -= l2;
			i2 += l2;
			l1 += l2;
		}
	...
	public void outlineSprite(int i, int k, int color) {
	...
	public void method354(Background background, int i, int j) {
		...
		if (i + i1 > DrawingArea.getBottomY())
			i1 -= (i + i1) - DrawingArea.getBottomY();
		...
		if (j + j1 > DrawingArea.getBottomX()) {
			int k2 = (j + j1) - DrawingArea.getBottomX();
			j1 -= k2;
			l1 += k2;
			k1 += k2;
		}
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(myPixels, j1, background.data, i1, DrawingArea.pixels, 0, k1, k, l1, l);
		}
	}
