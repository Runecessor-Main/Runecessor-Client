...
public final class ISAACRandomGen {
	...
	private void isaac() {
		...
		for (int i = 0; i < 256; i++) {
			...
			tmp185_184 = (this.memory[((j & 0x3FC) >> 2)] + this.accumulator + this.lastResult);
			int k = tmp185_184;
			this.memory[i] = tmp185_184;
			int tmp211_210 = (this.memory[((k >> 8 & 0x3FC) >> 2)] + j);
			this.lastResult = tmp211_210;
			this.results[i] = tmp211_210;
		}
	}
	...
	private void initializeKeySet() {
		...
		int j1;
		int i1;
		int l = i1 = j1 = k1 = l1 = i2 = j2 = k2 = -1640531527;
		for (int i = 0; i < 4; i++) {
			l ^= i1 << 11;
			k1 += l;
			i1 += j1;
