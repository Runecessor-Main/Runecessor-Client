...
public class DropdownMenu {
	...
	private void drawDropBox(RSInterface child, int x, int y, boolean hover) {
		...
		DrawingArea.fillPixels(x + 1, y + 1, width - 2, 22, 0);
		DrawingArea.method336(x + 2, y + 2, width - 4, 20, bgColor);

		RSFont font = Client.instance.newSmallFont;
		String message = !child.message.isEmpty() ? child.message : selected;

		x += child.centerText ? width / 2 - 2 : 6;
		font.drawBasicString1(message, x, y + 17, false, textColor, child.centerText);
	}

	private void drawOpenBox(RSInterface child, int x, int y) {
		Sprite scrollBar = Client.cacheSprite[29];//TODO check sprite id for scroll bar
		if (child.hovered) {
		...
		DrawingArea.fillPixels(x + width - 21, y + 3, 17, 17, 0);
		int len = options.length;
		if (len > 5)
			len = 5;
		DrawingArea.fillRectangle(x + 1, y + 22, width - 2, SELECT_HEIGHT * len + 3, 0);
		...
		for (int i = 0; i < len; i++) {
			...
			DrawingArea.fillRectangle(x + 2, y + yy + 22, width - 4 - (options.length > 5 ? 8 : 0), SELECT_HEIGHT, color);
		...
		if (options.length > 5) {
			...
			DrawingArea.fillRectangle(x + width - 9, y + 24, 7, 13 * len, 0x534A3E);
			DrawingArea.fillRectangle(x + width - 9, y + 25 + scrollPos, 7, scrollLength - 1, 0x777067);
		}
	}
	...
	public void drawDropdown(RSInterface child, int x, int y) {
		...
		if (open) {
			...
			Sprite scrollBar = Client.cacheSprite[30];
			if (child.hovered) {
				scrollBar.drawTransparentSprite(x + width - 20, y + 4, 125);
			}
			else {
			...
			DrawingArea.fillPixels(x + width - 21, y + 4, 17, 17, 0);
		}
	}
	...
	public void hover(RSInterface parent, RSInterface child, int hoverX, int hoverY, int xBounds, int yBounds) {
		...
		client.menuActionCmd3[client.menuActionRow] = child.id;
		client.menuActionCmd2[client.menuActionRow] = child.dropDownHover;
		client.menuActionCmd1[client.menuActionRow] = parent.id;
