...
public class RSFont extends DrawingArea {
	...
	public static String replaceOldWithCol(String s1) {
		s1 = s1.replaceAll("@red@", "<col=ff0000>");
		s1 = s1.replaceAll("@gre@", "<col=65280>");
		s1 = s1.replaceAll("@blu@", "<col=255>");
		s1 = s1.replaceAll("@yel@", "<col=ffff00>");
		s1 = s1.replaceAll("@dbl@", "<col=000080>"); //level up dark blue
		s1 = s1.replaceAll("@cya@", "<col=65535>");
		s1 = s1.replaceAll("@mag@", "<col=ff00ff>");
		s1 = s1.replaceAll("@whi@", "<col=ffffff>");
		s1 = s1.replaceAll("@lre@", "<col=ff9040>");
		s1 = s1.replaceAll("@dre@", "<col=800000>");
		s1 = s1.replaceAll("@bla@", "<col=0>");
		s1 = s1.replaceAll("@or1@", "<col=ffb000>");
		s1 = s1.replaceAll("@or2@", "<col=ff7000>");
		s1 = s1.replaceAll("@or3@", "<col=ff3000>");
		s1 = s1.replaceAll("@gr1@", "<col=c0ff00>");
		s1 = s1.replaceAll("@gr2@", "<col=80ff00>");
		s1 = s1.replaceAll("@gr3@", "<col=40ff00>");
		s1 = s1.replaceAll("@RED@", "<col=ff0000>");
		s1 = s1.replaceAll("@GRE@", "<col=65280>");
		s1 = s1.replaceAll("@BLU@", "<col=255>");
		s1 = s1.replaceAll("@YEL@", "<col=ff0000>");
		s1 = s1.replaceAll("@CYA@", "<col=65535>");
		s1 = s1.replaceAll("@MAG@", "<col=ff00ff>");
		s1 = s1.replaceAll("@WHI@", "<col=ffffff>");
		s1 = s1.replaceAll("@LRE@", "<col=ff9040>");
	...
	private boolean containsColImg(String string) {
		if (string.contains("<col=") || string.contains("<img=")) {
			return true;
		}
		return false;
	}

	public void drawBasicString(String string, int drawX, int drawY, boolean showCrown) {
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			// character 60 is < and 62 is > and [ is 91 and ] is 93
			if (character > 255) {
				character = 32;
			}
			if (character == 60 && containsColImg(string)) {
					...
					if (effectString.equals(startEffect)) {
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								else if (!showCrown && imageId <= 12) {
									icon.drawSprite(drawX, (drawY + baseCharacterHeight - iconModY) - 3, transparency);
								}
								drawX += icon.maxWidth;
							}
							catch (Exception e) {
				...
				if (startIndex == -1) {
					...
					if (strikethroughColor != -1) {
						DrawingArea.drawHorizontalLine(drawX, drawY + (int) ((double) baseCharacterHeight * 0.69999999999999996D), lineWidth, strikethroughColor);
					}
					if (underlineColor != -1) {
		...
		for (int currentCharacter = 0; currentCharacter < string.length(); currentCharacter++) {
			...
			if (character == 60) {
				...
				if (startIndex == -1) {
					...
					if (strikethroughColor != -1) {
						DrawingArea.drawHorizontalLine(drawX, drawY + (int) ((double) baseCharacterHeight * 0.7), i_109_, strikethroughColor);
					}
					if (underlineColor != -1) {
	...
	/**
	 * Put shadow at -1 to not show any shadow.
	 */
	public void drawBasicString1(String string, int drawX, int drawY, int color, int shadow) {
		if (string != null) {
			setColorAndShadow(color, shadow);
	...
	public void drawTransparentCharacter(int i, int i_11_, int i_12_, int i_13_, int i_14_, int i_15_, int i_16_, boolean bool) {
		...
		if (i_12_ + i_14_ > DrawingArea.getBottomY()) {
			i_14_ -= i_12_ + i_14_ - DrawingArea.getBottomY();
		}
		...
		if (i_11_ + i_13_ > DrawingArea.getBottomX()) {
			int i_23_ = i_11_ + i_13_ - DrawingArea.getBottomX();
			i_13_ -= i_23_;
			i_19_ += i_23_;
			i_18_ += i_23_;
		}
	...
	public void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_36_ + i_38_ > DrawingArea.getBottomY()) {
			i_38_ -= i_36_ + i_38_ - DrawingArea.getBottomY();
		}
		...
		if (i_35_ + i_37_ > DrawingArea.getBottomX()) {
			int i_46_ = i_35_ + i_37_ - DrawingArea.getBottomX();
			i_37_ -= i_46_;
			i_42_ += i_46_;
			i_41_ += i_46_;
		}
	...
	public void drawBasicString1(String string, int drawX, int drawY, boolean shadow, int color, boolean centered) {
		drawY -= baseCharacterHeight;

		if (centered) {
			drawX = drawX - getTextWidth(string) / 2;
		}
