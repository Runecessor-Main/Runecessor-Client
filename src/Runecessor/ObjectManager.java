...
final class ObjectManager {
	...
	/**
	 * Objects that players can click on and walk through.
	 */
	private final static int[] noClippableObjects =
	{1097, 1098, 1099, 20839, 13665, 13666, 13667, 13668, 13671};
		...
		for (int z = 0; z < 4; z++) {
			...
			for (int j4 = 1; j4 < sizeY - 1; j4++) {
				for (int j5 = 1; j5 < sizeX - 1; j5++) {
					int k6 = heightMap[z][j5 + 1][j4] - heightMap[z][j5 - 1][j4];
					int l7 = heightMap[z][j5][j4 + 1] - heightMap[z][j5][j4 - 1];
			...
			for (int x = -5; x < sizeX + 5; x++) {
				for (int i8 = 0; i8 < sizeY; i8++) {
					int k9 = x + 5;
					if (k9 >= 0 && k9 < sizeX) {
						int l12 = underlayFlo[z][k9][i8] & 0xff;
						if (l12 > 0) {
					...
					int i13 = x - 5;
					if (i13 >= 0 && i13 < sizeX) {
						int i14 = underlayFlo[z][i13][i8] & 0xff;
						if (i14 > 0) {
				...
				if (x >= 1 && x < sizeX - 1) {
					...
					for (int y = -5; y < sizeY + 5; y++) {
						...
						if (y >= 1 && y < sizeY - 1 && (!(lowMem) || (renderFlags[0][x][y] & 2) != 0 || (renderFlags[z][x][y] & 0x10) == 0 && getVisiblePlane(y, z, x) == buildPlane)) {
							if (z < minPlane) {
								minPlane = z;
							}
							int l18 = underlayFlo[z][x][y] & 0xff;
							int overlayId = overlayFlo[z][x][y] & 0xff;
							if (l18 > 0 || overlayId > 0) {
								int j19 = heightMap[z][x][y];
								int k19 = heightMap[z][x + 1][y];
								int l19 = heightMap[z][x + 1][y + 1];
								int i20 = heightMap[z][x][y + 1];
								int j20 = lightMap[x][y];
								int k20 = lightMap[x + 1][y];
								int l20 = lightMap[x + 1][y + 1];
								int i21 = lightMap[x][y + 1];
								int hsl = -1;
								int k21 = -1;
								if (l18 > 0) {
									int l21 = l9 * 256 / k15;
								...
								if (z > 0) {
									boolean hidden = true;
									if (l18 == 0 && overlayShape[z][x][y] != 0) {
										hidden = false;
									}
									if (overlayId > 0 && (Config.PRE_EOC ? !FloorOverlay.cache[overlayId -1].occlude_overlay : !FloorDefinition.overlays[overlayId - 1].occlude)) {
										hidden = false;
									}
									if (hidden && j19 == k19 && j19 == l19 && j19 == i20) {
										cullingMap[z][x][y] |= 0x924;
									}
								}
								int i22 = 0;
								if (hsl != -1) {
								...
								if (overlayId == 0) {
									worldController.addTile(z, x, y, 0, 0, -1, 154, j19, k19, l19, i20, adjustHSLLightness(hsl, j20), adjustHSLLightness(hsl, k20), adjustHSLLightness(hsl, l20), adjustHSLLightness(hsl, i21), 0, 0, 0, 0, i22, 0, false);
								}else {
								int shape;
								int textureId;
								byte shapeRot;
								...
								int floorId = -1;
							//	if(!Config.PRE_EOC) {
									shape = overlayShape[z][x][y] + 1;
									shapeRot = overlayRotation[z][x][y];
									FloorDefinition def = FloorDefinition.overlays[overlayId - 1];
									textureId = def.texture;
									...
									if (textureId >= 0) {
										minimapColor = Rasterizer.getAverageTextureRGB(textureId);
										floorId = -1;
									} else if (def.rgb == 0xff00ff) {
										minimapColor = 0;
										floorId = -2;
										textureId = -1;
									} else if (def.rgb == 0x333333) {
										minimapColor = Rasterizer.palette[getRGB(def.hslToRgb, 96)];
										floorId = -2;
										textureId = -1;
									} else {
										floorId = getTrimmedHSL(def.hue, def.saturation, def.lumiance);
										minimapColor = Rasterizer.palette[getRGB(def.hslToRgb, 96)];
									}
									if (minimapColor == 0x000000 && def.anotherRgb != -1) {
										int newMinimapColor = getTrimmedHSL(def.anotherHue, def.anotherSaturation, def.anotherLuminance);
										minimapColor = Rasterizer.palette[getRGB(newMinimapColor, 96)];
									}
									if (textureId == -1) {
										textureId = 154;
									}
							/*	} else {
									FloorOverlay floorOverlay = FloorOverlay.cache[overlayId - 1];
									shape = overlayShape[z][x][y] + 1;
									shapeRot = overlayRotation[z][x][y];
									textureId = floorOverlay.ground_texture_overlay;
									if(textureId > 50)
										textureId = -1;
									if (textureId >= 0) {
										minimapColor = Rasterizer.getAverageTextureRGB(textureId);
										floorId = -1;
									} else if (floorOverlay.ground_color_overlay == 0xff00ff) {//pink
										minimapColor = 0;
										floorId = -2;
										textureId = -1;
									} else {
										floorId = getTrimmedHSL(floorOverlay.ground_hue_overlay, floorOverlay.ground_sat_overlay, floorOverlay.ground_lum_overlay);
										minimapColor = Rasterizer.palette[getRGB(floorOverlay.hsl_blended_overlay, 96)];
									}*/
							/*		if((overlayId) > FloorDefinition.underlays.length)
										overlayId = FloorDefinition.underlays.length;//42 = invisible
									if (textureId >= 0) {
										minimapColor = Rasterizer.getAverageTextureRGB(textureId);
										floorId = -1;
									} else if (floorOverlay.ground_color_overlay == 0xff00ff) {//pink
										...
										floorId = -2;
										textureId = -1;
									} else if(floorOverlay.ground_color_overlay == 0x333333) {//green
										minimapColor = Rasterizer.palette[getRGB(floorOverlay.hsl_blended_overlay, 96)];
										floorId = -2;
										textureId = -1;
									} else {
									...
									if((overlayId) == 54) {
										minimapColor = floorOverlay.ground_color_overlay= 0x828282;
										floorId = -2;
										textureId = -1;
									}
									if(overlayId == 111) {											//Water
										minimapColor =Rasterizer.getAverageTextureRGB(1);
										floorId = -1;
										textureId = 1;
									} else if(overlayId == 63) {										//Seer's court stairs
										minimapColor = floorOverlay.ground_color_overlay = 0x767676;
										floorId = -2;
										textureId = -1;
									} else if(overlayId == 177) {										//Castle Wars, lobby floor
										minimapColor = floorOverlay.ground_color_overlay = 0x4D4D4D;
										floorId = getTrimmedHSL(0,0,55);
										textureId = -1;
									} else if(overlayId == 72) {										//Cliffside at ogres
										minimapColor = floorOverlay.ground_color_overlay = 0x483B21;
										floorId = getTrimmedHSL(25,146,24);
									} else if (floorId == 6363 || floorId == 549) {						//Dirt banks, etc
										minimapColor = 0x483B21;
										floorId = getTrimmedHSL(25,146,24);
									} else if (floorId == 40) {//Grey roads
										minimapColor = floorOverlay.ground_color_overlay;
										textureId = -1;
									}*/
								//}
									worldController.addTile(z, x, y, shape, (int) shapeRot, textureId, 154, j19, k19, l19, i20, adjustHSLLightness(hsl, j20), adjustHSLLightness(hsl, k20), adjustHSLLightness(hsl, l20), adjustHSLLightness(hsl, i21), getRGB(floorId, j20), getRGB(floorId, k20), getRGB(floorId, l20), getRGB(floorId, i21), i22, minimapColor, false);
								}
							}
						}
					}
			...
			for (int j8 = 1; j8 < sizeY - 1; j8++) {
				for (int i10 = 1; i10 < sizeX - 1; i10++) {
					worldController.setTopPlane(z, i10, j8, getVisiblePlane(j8, z, i10));
				}
	...
	private void addObject(int i, Landscape worldController, CollisionMap class11, int type, int k, int l, int objectId, int j1) {
		...
		if (type == 22) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj = new SceneObject(objectId, j1, 22, l1, i2, k1, j2, class46.animationID, true);
			}
		...
		if (type == 10 || type == 11) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj1 = new SceneObject(objectId, j1, 10, l1, i2, k1, j2, class46.animationID, true);
			}
			if (obj1 != null) {
				int i5 = 0;
				if (type == 11)
		...
		if (type >= 12) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj2 = new SceneObject(objectId, j1, type, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 0) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj3 = new SceneObject(objectId, j1, 0, l1, i2, k1, j2, class46.animationID, true);
			}
		...
		if (type == 1) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj4 = new SceneObject(objectId, j1, 1, l1, i2, k1, j2, class46.animationID, true);
			}
		...
		if (type == 2) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj11 = new SceneObject(objectId, 4 + j1, 2, l1, i2, k1, j2, class46.animationID, true);
				obj12 = new SceneObject(objectId, i3, 2, l1, i2, k1, j2, class46.animationID, true);
			}
		...
		if (type == 3) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj5 = new SceneObject(objectId, j1, 3, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 9) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj6 = new SceneObject(objectId, j1, type, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 4) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj7 = new SceneObject(objectId, 0, 4, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 5) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj13 = new SceneObject(objectId, 0, 4, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 6) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj8 = new SceneObject(objectId, 0, 4, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 7) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj9 = new SceneObject(objectId, 0, 4, l1, i2, k1, j2, class46.animationID, true);
		...
		if (type == 8) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj10 = new SceneObject(objectId, 0, 4, l1, i2, k1, j2, class46.animationID, true);
	...
	public static void addObject(Landscape worldController, int i, int j, int k, int l, CollisionMap class11, int ai[][][], int i1, int objectId, int k1) {
		...
		if (k == 22) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj = new SceneObject(objectId, i, 22, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 10 || k == 11) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj1 = new SceneObject(objectId, i, 10, i2, j2, l1, k2, class46.animationID, true);
			if (obj1 != null) {
				int j5 = 0;
				if (k == 11)
					j5 += 256;
			...
			if (class46.blocksWalk) {
				for (int index = 0; index < noClippableObjects.length; index++) {
					if (objectId == noClippableObjects[index]) {
						return;
					}
				}
		...
		if (k >= 12) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj2 = new SceneObject(objectId, i, k, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 0) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj3 = new SceneObject(objectId, i, 0, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 1) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj4 = new SceneObject(objectId, i, 1, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 2) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj11 = new SceneObject(objectId, 4 + i, 2, i2, j2, l1, k2, class46.animationID, true);
				obj12 = new SceneObject(objectId, j3, 2, i2, j2, l1, k2, class46.animationID, true);
			}
		...
		if (k == 3) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj5 = new SceneObject(objectId, i, 3, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 9) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj6 = new SceneObject(objectId, i, k, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 4) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj7 = new SceneObject(objectId, 0, 4, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 5) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj13 = new SceneObject(objectId, 0, 4, i2, j2, l1, k2, class46.animationID, true);
			}
		...
		if (k == 6) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null) {
				...
				obj8 = new SceneObject(objectId, 0, 4, i2, j2, l1, k2, class46.animationID, true);
			}
		...
		if (k == 7) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj9 = new SceneObject(objectId, 0, 4, i2, j2, l1, k2, class46.animationID, true);
		...
		if (k == 8) {
			...
			if (class46.animationID == -1 && class46.childrenIDs == null)
				...
				obj10 = new SceneObject(objectId, 0, 4, i2, j2, l1, k2, class46.animationID, true);
	...
	private static final int ROT_X_DELTA[] =
	{1, 0, -1, 0};
	...
	private static final int ROT_Y_DELTA[] =
	{0, -1, 0, 1};
