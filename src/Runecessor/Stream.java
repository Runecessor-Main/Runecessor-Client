...
public final class Stream extends NodeSub {
	...
	public int readThreeBytesMiddleTailReverse() {
		currentOffset += 3;
		return ((buffer[currentOffset - 3] & 0xff) << 16) + (buffer[currentOffset - 1] & 0xff) + ((buffer[currentOffset - 2] & 0xff) << 8);
	}
	...
	public int method421() {
		int i = buffer[currentOffset] & 0xff;//ArrayIndex out of bounds exception
		if (i < 128)
...
/*Useful conversions:
 * from osrs format to 317:
 * readUShort() = readUnsignedWord()
 * readUByte() = readUnsignedByte()
 * read24Int() = read3Bytes()
 */