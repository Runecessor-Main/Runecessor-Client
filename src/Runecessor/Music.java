...
public class Music {
    ...
    public static final Music[] songs =
            {
                    ...
                    new Music("Autumn Voyage", 3200, 3303, 3273, 3353, 2, 4304, 1),
		...
		new Music("Goblin Game", 2560, 9792, 2623, 9855, 346, 11106, 128),
                    ...
                    new Music("Into the Runecessor", 3008, 4800, 3071, 4927, 412, 13712, 149),
    ...
    private static int interfaceIdsToChangeColour[] =
            {5450, 6271, 6272, 9926, 3206};
    public int music, swX, swY, neX, neY, interfaceId, array;
    public String name;
    ...
    public Music(String name, int swX, int swY, int neX, int neY, int music, int tabId, int array) {
        ...
        this.neY = neY;
        this.music = music;
        this.interfaceId = tabId;
        this.array = array;
    }

    public static void playLogInScreenMusic() {
        if (Client.musicVolume == 0) {
            return;
        }
        Client.nextSong = 16; //(normal theme)
		//Client.nextSong = 547; //Scape Santa (xmas)
		//Client.nextSong = 321; //Scape Scared (h'ween)

        Client.onDemandFetcher.sendImmediately(2, Client.nextSong);
    }

    public static void soundSystem() {
        if (System.currentTimeMillis() - Client.lastSoundTime <= 20) {
            return;
        }
        for (int index = 0; index < Client.currentSound; index++) {
            if (System.currentTimeMillis() - Client.lastSoundTime <= 20) {
                break;
            }
            try {
                Stream stream = WaveSound.get(Client.soundType[index], Client.sound[index]);

                if (stream != null) {
                    new SoundPlayer((InputStream) new ByteArrayInputStream(stream.buffer, 0, stream.currentOffset), Client.soundVolume[index], Client.soundDelay[index]);
                    Client.lastSoundTime = System.currentTimeMillis();
                }
            } catch (Exception exception) {
                exception.printStackTrace();
            }
            Client.currentSound--;
            for (int j = index; j < Client.currentSound; j++) {
                Client.sound[j] = Client.sound[j + 1];
                Client.soundType[j] = Client.soundType[j + 1];
                Client.soundDelay[j] = Client.soundDelay[j + 1];
                Client.soundVolume[j] = Client.soundVolume[j + 1];
            }
            index--;
        }
    ...
    public static void packMusic() {
        for (int MusicIndex = 0; MusicIndex < 3536; MusicIndex++) {
            byte[] abyte0 = packMusic(MusicIndex);
            if (abyte0 != null && abyte0.length > 0) {
                if (Config.PRE_EOC) {
                    Client.instance.decompressors[3].put(abyte0.length, java.nio.ByteBuffer.wrap(abyte0), MusicIndex);
                } else {
                    Client.instance.decompressors[3].method234Osrs(abyte0.length, abyte0, MusicIndex);
                }
            }
        }
    }

    public static byte[] packMusic(int Index) {
        try {
			File Music = new File(ClientConstants.getMainCacheLocation() + "Music/" + Index + ".gz");
            byte[] aByte = new byte[(int) Music.length()];
            FileInputStream Fis = new FileInputStream(Music);
            Fis.read(aByte);
            Utility.print("" + Index + " aByte = [" + aByte + "]!");
            Fis.close();
            return aByte;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * Inform the server on the music state, weather it's manual or automatic.
     */
    public static void updateServerMusicState() {
        if (Client.autoMusic) {
            Client.instance.sendCommandPacket("::musicauto");
        } else {
            Client.instance.sendCommandPacket("::musicmanual");
        }
        if (Client.loop) {
            Client.instance.sendCommandPacket("::musicloopon");
        } else {
            Client.instance.sendCommandPacket("::musicloopoff");
        }
    }

    public static void playNextSong() {
        if (Client.forceDisableMidiSystem) {
            return;
        }
        if (!midiPlayer.playing() && !Client.loop && Client.musicVolume > 0 && !Client.sentRequestToPlayNextSong && Client.permissionToPlayNextSong && System.currentTimeMillis() - Client.songChangeDelay > 4000 && Client.isLoggedIn()) {
            Client.sentRequestToPlayNextSong = true;
            Client.songChangeDelay = System.currentTimeMillis();
            Client.instance.sendCommandPacket("::musicnext");
        }

    }

    public static void updateMusicInterface() {
        Client.instance.variousSettings[18] = Client.autoMusic ? 1 : 0;
        Client.instance.variousSettings[19] = Client.loop ? 1 : 0;
    }

    public static void turnMusicTabGreen() {
        for (int i = 0; i < songs.length; i++) {
            Client.instance.changeColour(songs[i].interfaceId, 8160);
        }
    }

    /**
     * Change the yellow text into orange.
     */
    public static void modifyMusicTextColour(RSInterface rsInterface) {
        for (int a = 0; a < interfaceIdsToChangeColour.length; a++) {

            if (rsInterface.id == interfaceIdsToChangeColour[a]) {
                rsInterface.textColour = 0xff9b00;
            }
        }
    }

    public static void addHoverToMusicInterface(RSInterface rsInterface) {
        for (int a = 0; a < Music.songs.length; a++) {
            if (rsInterface.id == Music.songs[a].interfaceId) {
    ...
    public static void stopMidi() {
        ...
        midiPlayer.stop();
        Client.permissionToPlayNextSong = false;
    }
    ...
    static void setMidiVolume(int vol) {
        ...
        Client.midiVolume = vol;
        if (midiPlayer.playing()) {
            midiPlayer.setVolume(0, Client.midiVolume);
        }
    }

    public static void playMidi(byte abyte0[]) {
        if (Client.forceDisableMidiSystem) {
            return;
        }
        boolean loopMusic = Client.loop;
        midiPlayer.play(abyte0, loopMusic, Client.midiVolume);
        if (!Client.isLoggedIn()) // Main theme playing at the log-in screen.
        {
            return;
        }
        Client.permissionToPlayNextSong = true;
    }
