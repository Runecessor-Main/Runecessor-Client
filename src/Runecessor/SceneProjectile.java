...
final class SceneProjectile extends SceneNode {
	...
	public void update(int i, int j, int k, int l) {
		if (!isMobile) {
			...
			startX = (double) x0 + (d * (double) arcScale) / d3;
			startY = (double) y0 + (d2 * (double) arcScale) / d3;
			startElevation = z0;
		}
		double d1 = (endCycle + 1) - i;
		velocityX = ((double) l - startX) / d1;
		velocityY = ((double) j - startY) / d1;
		velocity = Math.sqrt(velocityX * velocityX + velocityY * velocityY);
		if (!isMobile)
			velocityZ = -velocity * Math.tan((double) elevationPitch * 0.02454369D);
		accelerationZ = (2D * ((double) k - startElevation - velocityZ * d1)) / (d1 * d1);
	}
	...
	public Model getRotatedModel() {
		...
		if (spotanim.animationSequence != null)
			j = spotanim.animationSequence.primary[anInt1593];
	...
	public void update(int i) {
		isMobile = true;
		startX += velocityX * (double) i;
		startY += velocityY * (double) i;
		startElevation += velocityZ * (double) i + 0.5D * accelerationZ * (double) i * (double) i;
		...
		if (spotanim.animationSequence != null)
			for (anInt1594 += i; anInt1594 > spotanim.animationSequence.getDuration(anInt1593);) {
				anInt1594 -= spotanim.animationSequence.getDuration(anInt1593) + 1;
				anInt1593++;
				if (anInt1593 >= spotanim.animationSequence.length)
					anInt1593 = 0;
			}

	}

	public final int anInt1571; // Cannot find refactored.
	...
	public final int anInt1583; // Cannot find refactored.
	...
	private int anInt1593; // Cannot find refactored.

	private int anInt1594; // Cannot find refactored.
