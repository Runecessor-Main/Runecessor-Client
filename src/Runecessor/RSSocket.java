...
final class RSSocket implements Runnable {
	...
	public void close() {
		...
		try {
			...
			e.printStackTrace();
			Utility.print("Error closing stream");
		}
		isWriter = false;
		synchronized (this) {
			notify();
	...
	public void printDebug() {
		Utility.print("dummy:" + closed);
		Utility.print("tcycl:" + writeIndex);
		Utility.print("tnum:" + buffIndex);
		Utility.print("writer:" + isWriter);
		Utility.print("ioerror:" + hasIOError);
		try {
			Utility.print("available:" + available());
		}
		catch (IOException e) {
			e.printStackTrace();
		}
